...
): Partial<typeof AssistantState.State> {
...
function buildPlanningNotes(
	...
	const sourceInstruction = [
		tools.useWebSearch ? 'Use web-search findings for live or external context.' : '',
		tools.useRag ? 'Use workspace retrieval findings for project-specific context.' : '',
	]
		.filter(Boolean)
		.join(' ');
